...
- [ ] **Verify `TG_BOT_TOKEN` in tg-web's deployed `.env` is byte-identical to `BOT_TOKEN` in tg-bot's deployed `.env`.** This is not checked anywhere at runtime — a mismatch makes every Mini App login silently 403 with nothing crashing anywhere, since Telegram signs `initData` per-bot. `bot.js` now logs `menu button set, web_app url: <url>` on successful startup (a final-review addition) — seeing that line confirms tg-bot itself booted with a valid token and the right URL, but does NOT confirm tg-web's copy of the token matches; that still needs a direct side-by-side comparison of the two `.env` files.
